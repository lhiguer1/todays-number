...
class AddTitleDateKeyPP(PostProcessor):
    """Add a `title_date` key to infodict with the value of the date found in title in iso format."""
    def run(self, info:dict):
        ...
        info['title_date'] = d.isoformat()

        self.to_screen(f'`title_date` added to infodict with value `{info["title_date"]}`')
        return [], info 
